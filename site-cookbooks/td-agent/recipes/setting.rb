...
template '/etc/td-agent/conf.d/nginx_access_log.conf' do
  variables mackerel_api_key: mackerel_credentials['api_key_old'], mackerel_service_name: 'a-know-home'
  source 'nginx_access_log.conf.erb'
  notifies :restart, 'service[td-agent]'
end

template '/etc/td-agent/conf.d/grass_graph_nginx_access_log.conf' do
  variables mackerel_api_key: mackerel_credentials['api_key_old'], mackerel_service_name: 'grass-graph'
